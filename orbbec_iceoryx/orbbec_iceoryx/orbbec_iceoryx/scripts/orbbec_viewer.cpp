...
int main(int argc, char **argv) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (arg == "--name" && i + 1 < argc) {
      ...
      std::cout
          ...
          << "  --mjpeg              iceoryx MJPEG stream "
             "(Orbbec/<name>/MJPEG)\n"
          << "  --mjpeg-stream       HTTP MJPEG stream (use with --mjpeg-url)\n"
          << "  --mjpeg-url <url>    HTTP MJPEG URL (default: "
             "http://localhost:9000/)\n"
  ...
  auto updateFps = [&](const std::string &windowTitle) {
    ...
    if (elapsed >= 1.0) {
      ...
      frameCount = 0;
      lastFpsTime = now;
      cv::setWindowTitle(windowTitle, windowTitle + "  |  " +
                                          std::to_string(static_cast<int>(
                                              std::round(currentFps))) +
                                          " fps");
      log.info("viewer",
               fmt::format("[{}] FPS: {:.1f}", windowTitle, currentFps));
    }
  };

  if (useMjpegStream) {
    log.info("viewer",
             fmt::format("Connecting to HTTP MJPEG stream: {}", mjpegUrl));
    cv::VideoCapture cap(mjpegUrl);
    if (!cap.isOpened()) {
      log.error("viewer",
                fmt::format("Could not open MJPEG stream at {}", mjpegUrl));
      log.shutdown();
      return 1;
    }
    ...
    while (gRun.load()) {
      if (!cap.read(frame)) {
        log.warn("viewer", "Failed to grab frame from MJPEG stream");
    ...
    if (useMjpeg) {
      log.info(
          "viewer",
          fmt::format("Subscribing to iceoryx MJPEG: Orbbec/{}/MJPEG", name));
        ...
        subscriber.take()
            .and_then([&](const void *payload) {
              ...
              if (frame && frame->data_size > 0) {
                std::vector<uint8_t> jpegBuf(frame->data,
                ...
                if (!img.empty()) {
                  ...
                  cv::putText(img, cv::format("FPS: %.1f", currentFps),
                              ...
                              cv::Scalar(0, 255, 0), 2);
                  cv::putText(img, name + " [MJPEG]", cv::Point(25, 95),
      ...
      log.info(
          "viewer",
          fmt::format("Subscribing to iceoryx Frame: Orbbec/{}/Frame", name));
  ...
  log.info("viewer", "Viewer exiting.");
  log.shutdown();
  return 0;
}
